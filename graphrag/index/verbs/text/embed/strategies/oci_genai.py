...
from graphrag.llm import EmbeddingLLM, OCIGenAIConfiguration
...
async def run(
    ...
    batch_max_tokens = args.get("batch_max_tokens", 400)
    oci_genai_config = OCIGenAIConfiguration(llm_config)
    splitter = _get_splitter(oci_genai_config, batch_max_tokens)
    llm = _get_llm(oci_genai_config, callbacks, cache)
...
def _get_splitter(
    config: [OCIGenAIConfiguration], batch_max_tokens: int
) -> TokenTextSplitter:
