...
from graphrag_original.config.enums import LLMType
from graphrag_original.llm import (
    CompletionLLM,
    EmbeddingLLM,
    LLMCache,
    LLMLimiter,
...
if TYPE_CHECKING:
    ...
    from graphrag_original.index.cache import PipelineCache
    from graphrag_original.index.typing import ErrorHandlerFn
...
def _load_oci_genai_completion_llm(
    on_error: ErrorHandlerFn,
    cache: LLMCache,
    config: dict[str, Any],
    azure=False,
):
    return _create_oci_genai_completion_llm(
        OCIGenAIConfiguration({
            **_get_base_config(config),
            "model": config.get("model", "gpt-4-turbo-preview"),
            "deployment_name": config.get("deployment_name"),
...
def _load_oci_genai_chat_llm(
    ...
    return _create_oci_genai_chat_llm(
        OCIGenAIConfiguration({
            # Set default values
            **_get_base_config(config),
            "model": config.get("model", "cohere.command-r-plus"),
            "model_id": config.get("model_id", "cohere.command-r-plus"),
            "endpoint": config.get("endpoint"),
            "config_profile": config.get("config_profile"),
...
def _load_oci_genai_embeddings_llm(
    on_error: ErrorHandlerFn,
    cache: LLMCache,
    config: dict[str, Any],
    azure=False,
    ...
    return _create_oci_genai_embeddings_llm(
        OCIGenAIConfiguration({
            **_get_base_config(config),
            "model": config.get(
                "embeddings_model", config.get("model", "cohere.embed-multilingual-v3.0")
            ),
            "model_id": config.get("model_id", "cohere.embed-multilingual-v3.0"),
            "config_profile": config.get("config_profile"),
            "compartment_id": config.get("compartment_id"),
            "endpoint": config.get("endpoint"),
            "deployment_name": config.get("deployment_name"),
...
loaders = {
    ...
    LLMType.OCIGenAI: {
        "load": _load_oci_genai_completion_llm,
        "chat": False,
    },
    LLMType.OpenAIChat: {
    ...
    LLMType.OCIGenAIChat: {
        "load": _load_oci_genai_chat_llm,
        "chat": True,
    },
    ...
    LLMType.OCIGenAIEmbedding: {
        "load": _load_oci_genai_embeddings_llm,
        "chat": False,
    },
...
def _create_oci_genai_chat_llm(
    ...
    limiter = _create_limiter(configuration)
    semaphore = _create_semaphore(configuration)
    return create_oci_genai_chat_llm(
        client, configuration, cache, limiter, semaphore, on_error=on_error
    )
...
def _create_oci_genai_completion_llm(
    configuration: OCIGenAIConfiguration,
    on_error: ErrorHandlerFn,
    cache: LLMCache,
    azure=False,
    ...
    limiter = _create_limiter(configuration)
    semaphore = _create_semaphore(configuration)
    return create_oci_genai_completion_llm(
        client, configuration, cache, limiter, semaphore, on_error=on_error
    )
...
def _create_oci_genai_embeddings_llm(
    configuration: OCIGenAIConfiguration,
    on_error: ErrorHandlerFn,
    cache: LLMCache,
    azure=False,
    ...
    limiter = _create_limiter(configuration)
    semaphore = _create_semaphore(configuration)
    return create_oci_genai_embedding_llm(
        client, configuration, cache, limiter, semaphore, on_error=on_error
    )


def _create_limiter(configuration: [OpenAIConfiguration, OCIGenAIConfiguration]) -> LLMLimiter:
...
def _create_semaphore(configuration: [OpenAIConfiguration, OCIGenAIConfiguration]) -> asyncio.Semaphore | None:
    limit_name = configuration.model or configuration.deployment_name or "default"
